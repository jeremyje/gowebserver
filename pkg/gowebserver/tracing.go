...
func newTracingHTTPHandler(handler http.Handler, metricsEnabled bool) http.Handler {
...
func (thh *tracingHTTPHandler) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	...
	zap.S().With("method", request.Method, "path", request.URL.Path).Debug("HTTP Request")
